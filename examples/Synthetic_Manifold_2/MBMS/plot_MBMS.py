...
MBMS_data =   pd.read_csv('../Output/MBMS_output.csv', header=None).to_numpy()
...
print("\nPLOTTING MBMS OUTPUT OF THE EXAMPLE " + example_name.upper() )
...
sc = ax.scatter(selected_data[:,0],selected_data[:,1],selected_data[:,2], s=1.0)
ax.view_init(44, 125)
ax.set_title('Selected data from LAAT')
...
sc = ax.scatter(MBMS_data[:,0], MBMS_data[:,1], MBMS_data[:,2], s=1.0)
ax.view_init(44, 125)
ax.set_title("\nMBMS output")
ax.set_xlabel('X-axis'  , labelpad=10)
ax.set_ylabel('Y-axis'  , labelpad=10)
...
plt.savefig('../Images/3.-MBMS_output.png')
